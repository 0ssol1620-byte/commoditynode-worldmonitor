...
| `commoditynode-foundation.yml` | PR and push on the CommodityNode feature branch | Verifies the additive CommodityNode variant, source-offer contract, dedicated production build, and immutable build artifact |
| `commoditynode-upstream-sync.yml` | Weekly cron, manual | Fetches upstream and opens a review-only sync PR when the fork is behind; never auto-merges |
